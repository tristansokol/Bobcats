...
docker build -f  ppo2-cpu.docker -t $DOCKER_REGISTRY/ppo2-agent:v1 .&&

docker push $DOCKER_REGISTRY/ppo2-agent:v1 &&

retro-contest run --agent $DOCKER_REGISTRY/ppo2-agent:v1 \
